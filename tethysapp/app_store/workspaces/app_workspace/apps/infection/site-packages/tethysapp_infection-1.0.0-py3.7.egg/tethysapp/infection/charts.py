...
from .app import Infection as App
...
def newchart(data):
    ...
    path = os.path.join(App.get_custom_setting('thredds_path'))
    files = [path]

    # get the timeseries, units, and message based on location type
    if data['loc_type'] == 'Point':
        values, meta['units'], meta['seriesmsg'] = pointchart('Infection', data['coords'], path, files)
    elif data['loc_type'] == 'Polygon':
        values, meta['units'], meta['seriesmsg'] = polychart('Infection', data['coords'], path, files)
    else:  # loc_type == 'VectorGeometry':
        values, meta['units'], meta['seriesmsg'] = vectorchart('Infection', data['vectordata'], path,
