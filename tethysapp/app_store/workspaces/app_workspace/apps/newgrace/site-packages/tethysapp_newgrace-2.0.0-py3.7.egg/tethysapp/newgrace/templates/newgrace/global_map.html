...
{% block styles %}
{{ block.super }}
<link rel="stylesheet" href="{% static 'newgrace/css/main.css' %}" />
  ...
  <html>
    ...
    <script>window.onbeforeunload = null;</script>
        ...
        <span class="leg_min_max" style="white-space:nowrap; float:right; margin-left:0px;">
                ...
                Min:
                    <input type="number" name="leg_min" id="col_min" min="-100" max="100" step="5" value="-50" onchange=updateWMS()>
                Max:
                    <input type="number" name="leg_max" id="col_max" min="-100" max="100" step="5" value="50" onchange=updateWMS()>
                Opacity:
                    <input type="number" name="opacity" id="opacity_val" min="0.1" max="1" step="0.1" value="0.7" onchange=updateWMS()>

        </span>
        <div id="map" thredds_wms={{thredds_wms}} style="height: 500px; width: 100%"></div>
        <br>
    ...
    <script type="text/javascript" src="{% static 'newgrace/js/graceviewer.js' %}"></script>
    </body>
</html>
...
<span style="white-space:nowrap; float:left; margin-left:2%; color:blue; cursor:pointer;" id= info-signal-button onclick=getSignalGuide() class="glyphicon glyphicon-info-sign">_</span>

{% gizmo select_input select_signal_process %}

<span style="white-space:nowrap; float:left; margin-left:2%; color:blue; cursor:pointer;" id= info-storage-button onclick=getStorageGuide() class="glyphicon glyphicon-info-sign">_</span>
...
<br>
<span style="white-space:nowrap; float:left; margin-left:2%; color:blue; cursor:pointer;" id= info-button onclick=getInstructions() class="glyphicon glyphicon-info-sign">_</span>
